...
class FusionModelv8(nn.Module):
    def __init__(self, wifi_feature_size, imu_feature_size, visual_feature_size, num_classes):
        ...
        self.matching_layer = nn.Sequential(
            nn.Linear(wifi_feature_size + imu_feature_size + visual_feature_size, 128),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Linear(128, 32),
            nn.BatchNorm1d(32),
    ...
    def forward(self, obs):
        ...
        visual_features = self.visual_feature_extractor(visual_data).view(batch_size, -1, num_tracklets,
                                                                        time_steps)
        ...
        combined_features = torch.cat((wifi_features_expanded, imu_features_expanded, visual_features), dim=1)

        # Reshape combined_features to (-1, imu_feature_size + visual_feature_size)
        combined_features = combined_features.view(-1, wifi_features_expanded.shape[1] + imu_features_expanded.shape[1] +
