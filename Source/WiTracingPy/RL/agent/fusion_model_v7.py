...
Fusion model v3. Input with 3 LSTM feature extractor and Conv1D layer to do matching

'''

class FusionModelv7(nn.Module):
    def __init__(self, wifi_feature_size, imu_feature_size, visual_feature_size, num_classes):
        super(FusionModelv7, self).__init__()
        # Wifi encoder
    ...
    def forward(self, obs):
        ...
        combined_features = combined_features.view(-1, wifi_features.shape[-1] + imu_features.shape[-1] +
                                                   visual_features.shape[-1], time_steps)
