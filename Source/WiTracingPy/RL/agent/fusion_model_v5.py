...
Fusion model v5. Input with 3 LSTM feature extractor and Gated Multimodal Unit as the fusing layer.
...
class GMU(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.linear_z = nn.Linear(input_size * 2, hidden_size)

    def forward(self, x1, x2):
        h1 = self.linear_h(x1)
        h2 = self.linear_h(x2)
        z = F.sigmoid(self.linear_z(torch.cat([x1, x2], dim=-1)))
        return z * h1 + (1 - z) * h2

class FusionModelv5(nn.Module):
    def __init__(self, wifi_feature_size, imu_feature_size, visual_feature_size, lstm_hidden_size,
                 max_pedestrian_detections, num_classes):
        super(FusionModelv5, self).__init__()
        # Wifi encoder
        ...
        self.attention = GMU(input_size=wifi_feature_size+imu_feature_size, hidden_size=64)
    ...
    def forward(self, obs):
        ...
        visual_data = visual_data.permute(0, 2, 1, 3).contiguous().view(batch_size * num_tracklets, time_steps,
                                                                        feature_size)
        ...
        out_feature = self.attention(combined_features, visual_features)
