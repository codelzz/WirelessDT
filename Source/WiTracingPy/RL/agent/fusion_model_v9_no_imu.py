...
class FusionModelv9_No_IMU(nn.Module):
    def __init__(self, wifi_feature_size, imu_feature_size, visual_feature_size, num_classes):
        ...
        self.matching_layer = nn.Sequential(
            nn.Linear(wifi_feature_size + visual_feature_size, 512),
            # nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Linear(512, 256),
            # nn.BatchNorm1d(256),
    ...
    def forward(self, obs):
        ...
        combined_features = torch.cat((wifi_features, visual_features), dim=-1)

        # Reshape combined_features to (-1, imu_feature_size + visual_feature_size)
        combined_features = combined_features.view(-1, wifi_features.shape[-1] +
