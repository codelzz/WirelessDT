...
class RLfuseAblaEnv(gym.Env):
    ...
    def get_distance(self,world_x, world_y, cam_x, cam_y):
        return math.sqrt((world_x - cam_x)**2 + (world_y - cam_y)**2) / 1000
    ...
    def _get_obs(self, curr_timestamp_idx):
        ...
        for i in range(curr_timestamp_idx, curr_timestamp_idx + self.timestamp_length, inteval):
            ...
            world_x = vis_data.iloc[0]['world_x']
            world_y = vis_data.iloc[0]['world_y']

            gt_angle = vis_data.iloc[0]['x']
            gt_distance = self.get_distance(world_x, world_y, self.cam_x, self.cam_y)
            ...
            is_turing = imu_data['is_turing'].tolist()[0]
            gt_actions.append(is_turing)
        ...
        gt_label = gt_distances, gt_angles, gt_actions

        return wifi_name_lists , wifi_rssi_lists, vis_lists, imu_list, label_list, gt_label

    def reset(self, seed=None, options=None):
        self.curr_timestamp_idx = random.randint(0, 35000)
        wifi_name_lists , wifi_rssi_lists, vis_list, imu_list, label_list, gt_label = self._get_obs(self.curr_timestamp_idx)
    ...
    def get_batch_obs(self, timestamp_idx, time_length):
        for i in range(timestamp_idx, timestamp_idx + time_length):
            wifi_name_lists, wifi_rssi_lists, vis_lists, imu_list, label_list, gt_label = self._get_obs(i)
    ...
    def get_evaluate_obs(self, timestep_idx):
        wifi_name_lists , wifi_rssi_lists, vis_list, imu_list, label_list, gt_label = self._get_obs(timestep_idx)
        obs = (wifi_name_lists , wifi_rssi_lists, imu_list, vis_list)
        info = {
            "label": label_list
        }
